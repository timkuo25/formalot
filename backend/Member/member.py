...
def addMember(user_email, user_firstname, user_lastname, student_id, user_hashed_pwd):
    ...
    try:
        ...
        '''
        cursor.execute(query, (user_email, user_firstname,
                       user_lastname, student_id, user_hashed_pwd))
        db.commit()
...
def Login():
    ...
    if login_check(email, password) == True:
        ...
        access_token = create_access_token(
            identity=id, expires_delta=timedelta(minutes=120))
...
def sendemail(recipient, condition):
    ...
    if condition == "register":
        ...
        msg.body = "請使用驗證碼開通您的 Formalot 帳號。 這是您的驗證碼: " + code
